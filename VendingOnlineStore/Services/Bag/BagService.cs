...
public class BagService : IBagService
{
    ...
    public async Task<IEnumerable<BagMachine>> GetContentAsync()
    {
        ...
        var bagMachines = content.Select(machine =>
        {
            ...
            var bagMachineItems = machine.Items.Select(item =>
            {
                ...
                var machineItemInfo = new Dtos.MachineItemInfo(machineItem.AvailableCount, machineItemPrice);
    ...
    private static Dtos.MachineInfo Map(Clients.Vending.Dtos.MachineInfo machineInfo)
    {
        var result = new Dtos.MachineInfo(machineInfo.Description, machineInfo.Address);
        return result;
    }

    private static Dtos.ItemInfo Map(Clients.Vending.Dtos.ItemInfo itemInfo)
    {
        var result = new Dtos.ItemInfo(itemInfo.Name, itemInfo.Description, itemInfo.PhotoLink);
        return result;
    }
}
